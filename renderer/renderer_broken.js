...
class ClipboardManager {
    constructor(appState) {
        ...
        // 注意：移除了定期检查剪切板的轮询，现在只依赖 Ctrl+C 事件监听    // 注意：checkClipboard 方法已移除，现在只使用基于事件的剪切板监控
    ...
    renderClipboardList() {
        ...
        this.appState.clipboardItems.forEach(item => {
    ...
    createClipboardItemElement(item) {
      ...
      <div class="clipboard-item-header">
        ...
        <div class="clipboard-item-actions">
          <button class="control-btn pin-btn" title="${item.pinned ? '取消置顶' : '置顶'}">
            ${item.pinned ? '📌' : '📍'}
          </button>
      ...
      <div class="clipboard-item-content">${this.escapeHtml(item.content)}</div>
      <div class="clipboard-item-time">${timeAgo}</div>
    `;
    ...
    editClipboardItem(itemId) {
        ...
        if (item) {
            ...
            if (newContent !== null && newContent !== item.content) {
                ...
                item.type = this.detectContentType(newContent);
                this.appState.saveData();
                this.renderClipboardList();
            }
        }
