...
export class WindowManager extends EventEmitter {
    ...
    // 储存窗口的原始尺寸，用于展开/收缩动画
    private originalWindowBounds: { width: number; height: number } | null = null;
    ...
    constructor(private config: WindowConfig, isDev = false) {
        ...
        this.window.on('close', (event) => {
            // 不阻止关闭事件，让应用直接退出
            logger.debug('Window close - application will exit');
        });
        ...
        this.window.on('resized', () => {
            ...
            if (newBounds && this.originalWindowBounds) {
                ...
                logger.debug('Window bounds updated after resize', {
    ...
    private showFromEdgeWithAnimation(): void {
        ...
        try {
            ...
            if (this.dockSide === 'top') {
                ...
                let lastFrameTime = 0; const animate = () => {
                    ...
                    if (progress < 1) {
                        ...
                        // 开始监听鼠标离开
                        this.startMouseLeaveMonitoring();
                        logger.debug(`Window expanded from top with animation in ${Date.now() - startTime}ms, final size: ${finalWidth}x${finalHeight}`);
                    }
                };
            ...
            if (this.dockSide === 'top') {
                ...
                const animate = () => {
                    ...
                    if (progress < 1) {
                        ...
                        this.hide();
                        logger.debug(`Window collapsed to top with animation in ${Date.now() - startTime}ms`);
                    }
                };
